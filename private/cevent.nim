...
when defined(windows):
  const
    mylib* = "libfswatch.dll"
elif defined(macosx):
  const
    mylib* = "libfswatch.dylib"
else:
  const
    mylib* = "libfswatch.so"
import times
...
type
  event_flag* {.size: sizeof(cint).} = enum
...
proc get_event_flag_by_name*(name: cstring; flag: ptr event_flag): STATUS {.cdecl,
    importc: "fsw_get_event_flag_by_name", dynlib: mylib.}
## *
...
proc get_event_flag_name*(flag: event_flag): cstring {.cdecl,
    importc: "fsw_get_event_flag_name", dynlib: mylib.}
## *
...
type
  CEVENT_CALLBACK* = proc (events: ptr cevent; event_num: cuint; data: pointer) {.cdecl.}
